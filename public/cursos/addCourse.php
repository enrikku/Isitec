...
if (isset($_FILES['image'])) {
    ...
    // Recorre los tags seleccionados
    foreach($tags_seleccionados as $tag) {
        echo "El tag seleccionado es: " . $tag . "<br>";
    }
    ...
    // Generar un valor aleatorio para incluir en el nombre del archivo
    $random_value = bin2hex(random_bytes(8)); // Genera 8 bytes de datos aleatorios y los convierte a una cadena hexadecimal

    // Concatenar el nombre original del archivo con el valor aleatorio
    $hashed_name = hash('sha256', $original_name . $random_value); // Usando SHA-256 como algoritmo hash
    ...
    if (move_uploaded_file($tmp_name, $destination)) {
        //guardarCurso($destination);
        echo "El archivo se subió correctamente como: $hashed_name.$extension";
    } else {
...
<form action="addCourse.php" method="post" enctype="multipart/form-data">
    <div class="mt-4 flex flex-col bg-gray-900 rounded-lg p-4">
        ...
        <div class="mt-4">
            ...
            <input placeholder="Enter a title for your dream" name="title" class="w-full bg-gray-800 rounded-md border-gray-700 text-white px-2 py-1" type="text">
        </div>

        <div class="mt-4">
            <label class="text-white" for="description">Descripcion</label>
            <textarea placeholder="Describe your dream in detail" name="description" class="w-full bg-gray-800 rounded-md border-gray-700 text-white px-2 py-1" id="description"></textarea>
        </div>
        ...
        <button type="submit" class="max-w-[140px] py-2 px-4 flex justify-center items-center  bg-green-600 hover:bg-green-500 focus:ring-green-500 focus:ring-offset-red-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg ">
